...
def get_data_by_frame(file_path, frame_idx):
    ...
    x_train = np.concatenate([np.real(x_data[0:L, :, :, :]), np.imag(x_data[0:L, :, :, :])], axis = 2)
    y_train = np.concatenate([np.real(y_data[0:L, :, :, :]), np.imag(y_data[0:L, :, :, :])], axis = 2)

    training_data = {'train_data':x_train, 'train_label':y_train}
...
def main():
    ...
    train_model(training_data, test_data, val_data, model, tf.train.AdamOptimizer(0.001),flags.train_url)
